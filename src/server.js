...
/* --- важное для Render --- */
app.set("trust proxy", 1);                 // доверяем прокси Render
app.get("/healthz", (req, res) => res.send("ok")); // healthcheck для Render
...
/* --- auth-мидлвар для API по initData --- */
app.use((req, res, next) => {
  // initData прилетает в заголовке из Mini App (см. public/app.js)
  const initData =
...
// Текущий пользователь + статус подписки
app.get("/api/me", (req, res) => {
  if (!req.tgUser) return res.status(401).json({ error: "Unauthorized" });

  const row = db
    .prepare(
      "SELECT is_premium, subscription_until FROM users WHERE user_id=?"
    )
    .get(req.tgUser.id);
  ...
  res.json({
    ...
    user: {
      ...
      is_premium:
        row?.is_premium === 1 && Number(row?.subscription_until ?? 0) > Date.now()
    }
  });
});
...
app.post("/api/tasks/:id/toggle", (req, res) => {
  ...
  const { id } = req.params;
  const row = db
    .prepare("SELECT done FROM tasks WHERE id=? AND user_id=?")
    .get(id, req.tgUser.id);
...
app.get("/api/habits", (req, res) => {
  if (!req.tgUser) return res.status(401).json({ error: "Unauthorized" });
  const items = db
    .prepare("SELECT * FROM habits WHERE user_id=? ORDER BY id DESC")
    .all(req.tgUser.id);
  res.json({ ok: true, items });
});
...
app.post("/api/habits", (req, res) => {
  ...
  const info = db
    .prepare(
      'INSERT INTO habits(user_id, title, streak, last_tick_date, created_at) VALUES(?, ?, 0, "", ?)'
    )
    .run(req.tgUser.id, title, Date.now());
  res.json({ ok: true, id: info.lastInsertRowid });
});
...
app.post("/api/habits/:id/tick", (req, res) => {
  ...
  if (h.last_tick_date !== today) {
    db.prepare(
      "UPDATE habits SET streak=streak+1, last_tick_date=? WHERE id=?"
    ).run(today, id);
  }
  res.json({ ok: true });
});
...
app.post("/api/goals", (req, res) => {
  ...
  const info = db
    .prepare(
      "INSERT INTO goals(user_id, title, progress, target, created_at) VALUES(?, ?, 0, ?, ?)"
    )
...
/* --- опционально: корень явно отдаём index.html --- */
app.get("/", (req, res) => {
  res.sendFile(path.join(__dirname, "..", "public", "index.html"));
});

/* --- старт сервера и бота --- */
